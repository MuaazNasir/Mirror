  ...
  return (
    <div className="fixed bottom-5 right-5 bg-conversation-panel-background border-2 border-green-400 py-3 px-5 m-2 rounded-xl flex flex-col gap-4 z-50">
      <div className="flex flex-row items-center justify-center gap-5">
        <Image src={incomingVoiceCall.profilePicture} alt="avatar" width={100} height={100} className="rounded-full border-2 border-gray-400" />
        <div className="flex flex-col items-center justify-start gap-2">
          <div className="text-gray-200 font-bold font-sans text-lg">{incomingVoiceCall.name}</div>
          <div className="text-secondary font-semibold text-md capitalize">incoming {incomingVoiceCall.callType} call</div>
        </div>
      </div>
      <div className="flex flex-row items-center justify-center gap-5">
